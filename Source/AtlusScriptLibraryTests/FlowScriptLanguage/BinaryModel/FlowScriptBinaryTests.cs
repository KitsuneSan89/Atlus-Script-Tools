...
namespace AtlusScriptLibrary.FlowScriptLanguage.BinaryModel.Tests
{
    ...
    public class FlowScriptBinaryTests
    {
        ...
        {
            var script = FlowScriptBinary.FromFile( Path.Combine("TestResources",$"{actualVersion}.bf"), version );
        ...
        {
            Assert.ThrowsException<InvalidDataException>( () => FlowScriptBinary.FromFile( Path.Combine("TestResources","dummy_small.bin"), BinaryFormatVersion.Unknown ) );
        }

        [TestMethod]
        public void FromFileTest_InvalidFileFormat_Big()
        {
            Assert.ThrowsException<InvalidDataException>( () => FlowScriptBinary.FromFile( Path.Combine("TestResources","dummy_big.bin"), BinaryFormatVersion.Unknown ) );
        }
