...
namespace AtlusScriptLibrary.FlowScriptLanguage.Tests
{
    ...
    public class FlowScriptTests
    {
        ...
        {
            var script = FlowScript.FromFile(Path.Combine("TestResources", $"{actualVersion}.bf"), null, version );
        ...
        {
            Assert.ThrowsException<InvalidDataException>( () => FlowScript.FromFile(Path.Combine("TestResources", "dummy_small.bin"), null, FormatVersion.Unknown ) );
        }

        [TestMethod]
        public void FromFile_ShouldThrowInvalidDataException_InvalidFileFormatBig()
        {
            Assert.ThrowsException<InvalidDataException>( () => FlowScript.FromFile(Path.Combine("TestResources", "dummy_big.bin"), null, FormatVersion.Unknown ) );
        }
        ...
        {
            using ( var fileStream = File.OpenRead(Path.Combine("TestResources", "Version3BigEndian.bf" ) ) )
            {
        ...
        {
            var binary = FlowScriptBinary.FromFile(Path.Combine("TestResources", "Version3BigEndian.bf"), BinaryFormatVersion.Version3BigEndian );
        ...
        {
            var binaryIn = FlowScriptBinary.FromFile(Path.Combine("TestResources", "Version3BigEndian.bf" ) );
