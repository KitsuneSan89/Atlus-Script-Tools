...
namespace AtlusScriptLibrary.MessageScriptLanguage.BinaryModel.Tests
{
    ...
    public class MessageScriptBinaryTests
    {
        ...
        {
            var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1.bmd") );
        ...
        {
            var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1.bmd"), BinaryFormatVersion.Version1 );
            PerformIntegrityCheckForVersion1( script );
        }
        ...
        {
            var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1.bmd"), BinaryFormatVersion.Version1BigEndian );
            PerformIntegrityCheckForVersion1( script );
        }
        ...
        {
            var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1BigEndian.bmd") );
        ...
        {
            var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1BigEndian.bmd"), BinaryFormatVersion.Version1BigEndian );
            PerformIntegrityCheckForVersion1BigEndian( script );
        }
        ...
        {
            var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1BigEndian.bmd"), BinaryFormatVersion.Version1 );
            PerformIntegrityCheckForVersion1BigEndian( script );
        }
        ...
        {
            Assert.ThrowsException<InvalidDataException>( () => MessageScriptBinary.FromFile( Path.Combine("TestResources","dummy_big.bin") ) );
        }
        ...
        {
            foreach ( var path in Directory.EnumerateFiles( Path.Combine("TestResources","Batch"), "*.bmd" ) )
            {
        ...
        {
            var script = MessageScriptBinary.FromStream( File.OpenRead( Path.Combine("TestResources","Version1.bmd") ) );
            PerformIntegrityCheckForVersion1( script );
        }
        ...
        {
            var script = MessageScriptBinary.FromStream( File.OpenRead( Path.Combine("TestResources","Version1BigEndian.bmd") ) );
            PerformIntegrityCheckForVersion1BigEndian( script );
        }
        ...
        public void ToFile_ResultShouldPassIntegrityCheck_Version1()
        {
            ...
            {
                var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1.bmd") );
                script.ToFile( Path.Combine("TestResources","Version1_ToFile.bmd") );
                script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1_ToFile.bmd") );
                PerformIntegrityCheckForVersion1( script );
            }
            finally
            {
                File.Delete( Path.Combine("TestResources","Version1_ToFileTest.bmd") );
            }
        }
        ...
        public void ToFile_ResultShouldPassIntegrityCheck_Version1BigEndian()
        {
            ...
            {
                var script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1BigEndian.bmd") );
                script.ToFile( Path.Combine("TestResources","Version1BigEndian_ToFile.bmd") );
                script = MessageScriptBinary.FromFile( Path.Combine("TestResources","Version1BigEndian_ToFile.bmd") );
                PerformIntegrityCheckForVersion1BigEndian( script );
            }
            finally
            {
                File.Delete( Path.Combine("TestResources","Version1BigEndian_ToFile.bmd") );
            }
        }
