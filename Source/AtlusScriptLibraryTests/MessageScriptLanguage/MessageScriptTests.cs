...
namespace AtlusScriptLibrary.MessageScriptLanguage.Tests
{
    ...
    public class MessageScriptTests
    {
        ...
        {
            var binary = MessageScriptBinary.FromFile( GetPath("Version1.bmd") );
        ...
        {
            var binary = MessageScriptBinary.FromFile( GetPath("Version1BigEndian.bmd")  );
            var script = MessageScript.FromBinary( binary );
        }
        ...
        {
            var script = MessageScript.FromFile(GetPath("Version1.bmd") );
        }

        [TestMethod]
        public void FromFile_ShouldNotThrow_Version1BigEndian()
        {
            var script = MessageScript.FromFile( GetPath("Version1BigEndian.bmd") );
        }

        [TestMethod]
        public void FromStream_ShouldNotThrow_Version1()
        {
            using ( var fileStream = File.OpenRead( GetPath("Version1.bmd" ) ) )
            {
        ...
        {
            using ( var fileStream = File.OpenRead( GetPath("Version1BigEndian.bmd" ) ) )
            {
                var script = MessageScript.FromStream( fileStream );
            }
        }
